...
export class RoadmapProposalEngine {
     ...
     */
    async generateProposals(signals, context) {
        if (signals.length === 0) {
            return [];
        }
     ...
     */
    proposalFromSignals(phase, signals, context) {
     ...
     */
    calculateImpact(phase, actions, context) {
     ...
     */
    calculateAffectedDependencies(phase, context) {
        const phaseInfo = context.currentPhases.find((p) => p.name === phase);
        if (!phaseInfo) {
            return [];
        }
     ...
     */
    generateRationale(signals, phase) {
     ...
     */
    calculateProposalConfidence(signals) {
        const avgConfidence = signals.reduce((sum, s) => sum + s.confidence, 0) / signals.length;
...
//# sourceMappingURL=RoadmapProposalEngine.js.map