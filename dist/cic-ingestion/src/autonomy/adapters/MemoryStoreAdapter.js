        ...
        return {
            ...
            payload: {
                ...
                metadata: {
                    ...
                    rationale: (signal.metadata && signal.metadata.rationale) || signal.rationale || '',
                },
            },
        };
    }
    ...
    static timelineEventToMemoryEvent(event, sessionId) {
        if (event.type === 'PIPELINE_RUN') {
            return {
                ...
                source_agent: event.source || event.metadata.source || 'unknown',
                session_id: sessionId,
                correlation_id: `corr_${generateUuid().substring(0, 8)}`,
                retention_days: 60,
                payload: {
                    pipeline_name: event.name || event.metadata.name || 'unnamed',
                    pipeline_id: event.id || generateUuid(),
                    status: event.status || event.metadata.status || 'pending',
                    ...
                    error_summary: event.error || event.metadata.error || undefined,
        ...
        return {
            ...
            retention_days: 30,
            payload: {
                agent_name: event.name || event.metadata.name || 'unknown',
                agent_class: 'autonomy',
                status: event.status || event.metadata.status || 'running',
                ...
                last_error: event.error || event.metadata.error || undefined,
