...
export class AutonomyToPlannerBridge {
     ...
     */
    async feedSignalsToPlanner(signals) {
        if (signals.length === 0) {
        ...
        try {
            ...
            console.log(`[PlannerBridge] Sent ${goals.length} goals to planner${shouldReplan ? ' (replan triggered)' : ''}`);
        }
        catch (err) {
     ...
     */
    async feedProposalToPlanner(proposal) {
        if (proposal.status !== 'approved') {
            console.log(`Proposal ${proposal.id} not approved, not feeding to planner`);
            return;
        }
        ...
        try {
            ...
            console.log(`[PlannerBridge] Applied constraints from proposal ${proposal.id}`);
        }
        catch (err) {
            ...
            switch (signal.type) {
                case 'drift':
                    goals.push({
                        ...
                        description: `Mitigate drift detected in ${signal.affectedPhases.join(', ')}: ${signal.description}`,
                ...
                case 'instability':
                    goals.push({
                        ...
                        type: 'stabilization',
                        description: `Stabilize ${signal.affectedPhases.join(', ')}: ${signal.description}`,
                        priority: this.signalSeverityToPriority(signal.severity) * signal.confidence,
                        targetPhase: signal.affectedPhases[0],
                        requiredActions: [
                            'allocate_resources',
                ...
                case 'regression':
                    goals.push({
                        ...
                        type: 'regression_fix',
                        description: `Fix regression in ${signal.affectedPhases.join(', ')}: ${signal.description}`,
                        priority: this.signalSeverityToPriority(signal.severity) * signal.confidence,
                        targetPhase: signal.affectedPhases[0],
                        requiredActions: [
                            'root_cause_analysis',
                ...
                case 'opportunity':
                    goals.push({
                        ...
                        type: 'acceleration',
                        description: `Accelerate ${signal.affectedPhases.join(', ')}: ${signal.description}`,
                        priority: (this.signalSeverityToPriority(signal.severity) * signal.confidence) / 2, // lower priority
                        targetPhase: signal.affectedPhases[0],
                        requiredActions: [
     ...
     */
    shouldTriggerReplan(signals, goals) {
     ...
     */
    convertProposalToConstraints(proposal) {
        const constraints = {
            proposalId: proposal.id,
            phaseConstraints: {},
            dependencies: [],
     ...
     */
    signalSeverityToPriority(severity) {
        switch (severity) {
            case 'critical':
                return 100;
            case 'warning':
     ...
     */
    async sendToPlannerControlPlane(request) {
        ...
        if (!response.ok) {
            throw new Error(`APR returned ${response.status}: ${response.statusText}`);
        }
    }
    /**
     * Send proposal constraints to APR control plane
     */
    async sendConstraintsToPlannerControlPlane(request) {
...
//# sourceMappingURL=AutonomyToPlannerBridge.js.map
