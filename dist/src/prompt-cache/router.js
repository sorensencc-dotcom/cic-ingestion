...
import { BatchOperationsManager } from './batch.js';
import { DEFAULT_CACHE_CONFIG } from './config/index.js';
export class CICPromptCacheRouter {
    constructor(config, registryDbPath) {
    ...
    async generateWithCache(opts) {
        const { docId, docText, userPrompt, model = this.config.model, maxTokens = 2000, } = opts;
        // Normalize document
        ...
        const { inputCost: inputPricing, cacheReadCost: cacheReadPricing, outputCost: outputPricing, } = this.config.pricingTiers;
        const inputCost = (inputTokens / 1000000) * inputPricing;
        const cacheReadCost = (cacheReadTokens / 1000000) * cacheReadPricing;
        const outputCost = (outputTokens / 1000000) * outputPricing;
        const totalCost = inputCost + cacheReadCost + outputCost;
        // Cost if no cache
        const costWithoutCache = ((inputTokens + cacheReadTokens) / 1000000) * inputPricing + outputCost;
    ...
    async generateBatchWithCache(req) {
