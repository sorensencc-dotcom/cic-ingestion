...
export class AutonomyGovernanceBridge {
    ...
    governanceHeaders() {
        ...
        if (!token) {
            throw new Error('[GovernanceBridge] CIC_GOVERNANCE_TOKEN is required but not set. ' +
     ...
     */
    async routeProposalToGovernance(proposal) {
        // Check if approval is needed
        if (!requiresGovernanceApproval(proposal)) {
            console.log(`[GovernanceBridge] Proposal ${proposal.id} does not require approval`);
            return;
        }
        ...
        try {
            ...
            console.log(`[GovernanceBridge] Routed proposal ${proposal.id} to Council for voting`);
        }
        catch (err) {
     ...
     * Record council vote
     */
    async recordVote(proposalId, voterId, decision, reason) {
        const vote = {
            proposalId,
            voterId,
            decision,
        ...
        try {
            ...
            console.log(`[GovernanceBridge] Recorded vote from ${voterId} on proposal ${proposalId}: ${decision}`);
        }
        catch (err) {
     ...
     */
    async finalizeDecision(proposal, votes) {
        ...
        const approvalPercentage = votingCount > 0 ? (approvals / votingCount) * 100 : 0;
        // Decide
        const approved = approvalPercentage >= this.config.approvalThreshold &&
        ...
        try {
            ...
            console.log(`[GovernanceBridge] Finalized proposal ${proposal.id}: ${decision.status}`);
        }
        catch (err) {
    ...
    async autoApproveProposal(proposal) {
        ...
        try {
            await this.recordDecision(decision);
            console.log(`[GovernanceBridge] Auto-approved proposal ${proposal.id} (no council vote needed)`);
        }
        catch (err) {
     ...
     */
    async submitToGovernance(request) {
        ...
        if (!response.ok) {
            throw new Error(`Governance returned ${response.status}: ${response.statusText}`);
        }
    }
    /**
...
//# sourceMappingURL=AutonomyGovernanceBridge.js.map