...
export class AutonomyToARPSBridge {
     ...
     */
    async logProposalToARPS(proposal) {
        const deltas = [];
        ...
        try {
            ...
            console.log(`[ARPSBridge] Logged proposal ${proposal.id} as ${deltas.length} ARPS_DELTA event(s)`);
        }
        catch (err) {
    ...
    async logSignalToARPS(signal) {
        ...
        try {
            await this.sendDeltaToARPS(delta);
            console.log(`[ARPSBridge] Logged signal ${signal.id} (${signal.type}) to ARPS`);
        }
        catch (err) {
     ...
     */
    async logProposalFeedbackToARPS(proposal, decision, reason) {
        const delta = {
        ...
        try {
            await this.sendDeltaToARPS(delta);
            console.log(`[ARPSBridge] Logged proposal feedback: ${proposal.id} → ${decision}`);
        }
        catch (err) {
     ...
     */
    async logProposalsToARPS(proposals) {
    ...
    async sendDeltaToARPS(delta) {
        ...
        if (!response.ok) {
            throw new Error(`MLA returned ${response.status}: ${response.statusText}`);
        }
    }
    /**
     * Helper: Map delta to event severity
     */
    getEventSeverity(delta) {
     ...
     */
    calculateProposalPriority(proposal) {
        let priority = 0;
...
//# sourceMappingURL=AutonomyToARPSBridge.js.map