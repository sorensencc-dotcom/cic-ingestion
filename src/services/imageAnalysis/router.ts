...
import { AnalyzeImageRequest, AnalyzeImageResponse, OcrRequest, OcrResponse } from './types';
...
export function createImageAnalysisRouter(config: ImageAnalysisConfig): Router {
  ...
  router.post('/analyze/ocr', async (req: Request, res: Response) => {
    try {
      const { imageBuffer, format, apiKey, requestId } = req.body;
      ...
      const reqId = requestId || generateUUID();
      const response: OcrResponse = await imageAnalysisService.ocr({
        imageBuffer,
        format,
        apiKey,
        requestId: reqId,
