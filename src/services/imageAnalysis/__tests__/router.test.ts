...
describe('ImageAnalysis Router - OCR', () => {
  let app: express.Application;
  let server: http.Server;
  let baseUrl: string;
  ...
  beforeEach(() => {
    ocrSpy = jest.spyOn(ImageAnalysisService.prototype, 'ocr');
  });
  ...
  it('validates that imageBuffer is present and is a non-empty string', async () => {
    const resMissing = await fetch(`${baseUrl}/api/analyze/ocr`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    });
  ...
  it('rejects oversized base64 strings exceeding limit', async () => {
    const oversizedBase64 = 'A'.repeat(70 * 1024 * 1024);
    const resLarge = await fetch(`${baseUrl}/api/analyze/ocr`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ imageBuffer: oversizedBase64 }),
    });
  ...
  it('returns text + metadata and preserves caller requestId', async () => {
    const pngBase64 = Buffer.from([0x89, 0x50, 0x4e, 0x47]).toString('base64');

    const res = await fetch(`${baseUrl}/api/analyze/ocr`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        imageBuffer: pngBase64,
        requestId: 'ocr-req-123',
      }),
    });
  ...
  it('auto-generates requestId when omitted', async () => {
    ...
    const res = await fetch(`${baseUrl}/api/analyze/ocr`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ imageBuffer: pngBase64 }),
    });

    expect(res.status).toBe(200);
    expect(ocrSpy).toHaveBeenCalled();
    const callArg = ocrSpy.mock.calls[0][0];
    expect(typeof callArg.requestId).toBe('string');
    expect(callArg.requestId.length).toBeGreaterThan(0);
  });

  it('handles errors thrown by ImageAnalysisService.ocr with 500 status', async () => {
    ocrSpy.mockRejectedValue(new Error('OCR service failure'));
    const pngBase64 = Buffer.from([0x89, 0x50, 0x4e, 0x47]).toString('base64');
    ...
    const body = (await res.json()) as { error: string; requestId: string };
    expect(body.error).toBe('Internal server error');
    expect(body.requestId).toBe('ocr-err-456');
  });
});
