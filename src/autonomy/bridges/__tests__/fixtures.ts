 ...
 */
export function createMockDriftSignal(
 ...
 */
export function createMockInstabilitySignal(
  severity: 'info' | 'warning' | 'critical' = 'warning'
): InstabilitySignal {
  return {
 ...
 */
export function createMockRegressionSignal(
  severity: 'info' | 'warning' | 'critical' = 'warning'
): RegressionSignal {
  return {
 ...
 */
export function createMockTimelineEvent(
  type:
    | 'ARPS_DELTA'
    | 'PIPELINE_RUN'
    | 'GOVERNANCE_SIGNAL'
    | 'APR_PLAN'
    | 'CRO_RUN' = 'PIPELINE_RUN'
): TimelineEvent {
  return {
 ...
 */
export function createMockProposalWithMultipleActions(): RoadmapProposal {
 ...
 */
export function createMockApprovedProposal(): RoadmapProposal {
 ...
 */
export function createMockSignals(count: number = 3): AutonomySignal[] {
  const signals: AutonomySignal[] = [];

  for (let i = 0; i < count; i++) {
    const type = ['drift', 'instability', 'regression', 'opportunity'][
      i % 4
    ] as any;
 ...
 */
export function createMockProposals(count: number = 2): RoadmapProposal[] {
  const proposals: RoadmapProposal[] = [];

  for (let i = 0; i < count; i++) {
    const status = i % 3 === 0 ? 'pending' : i % 3 === 1 ? 'approved' : 'rejected';
    proposals.push(createMockProposal(status as any));
  }
