...
export class SignalDetectionEngine {
   ...
   */
  async detectSignals(context: SignalDetectionContext): Promise<AutonomySignal[]> {
   ...
   */
  private detectDriftSignals(context: SignalDetectionContext): DriftSignal[] {
    ...
    if (avgDrift.combined_score > SIGNAL_THRESHOLDS.DRIFT_CRITICAL) {
      const affectedEvents = context.events.filter(
        (e) =>
      ...
      const signal: DriftSignal = {
        ...
        recommendation:
          'Review recent changes in ARPS, APR, and CRO. Consider roadmap adjustments.',
   ...
   */
  private detectInstabilitySignals(context: SignalDetectionContext): InstabilitySignal[] {
    const signals: InstabilitySignal[] = [];

    if (!context.healthMetrics || context.healthMetrics.length === 0) {
      return signals;
    }

    const recentMetrics = context.healthMetrics.filter(
      (m) =>
        new Date(m.timestamp).getTime() > Date.now() - 24 * 60 * 60 * 1000
    );

    const avgErrorRate =
      recentMetrics.reduce((sum, m) => sum + (1 - m.successRate), 0) /
    ...
    if (avgErrorRate > SIGNAL_THRESHOLDS.INSTABILITY_ERROR_RATE) {
      ...
      const signal: InstabilitySignal = {
        ...
        recommendation:
          'Investigate recent errors. Check PIPELINE_RUN and CRO_RUN events for failures.',
        instabilityMetrics: {
   ...
   */
  private detectRegressionSignals(context: SignalDetectionContext): RegressionSignal[] {
    ...
    const avgLatency =
      recentMetrics.reduce((sum, m) => sum + m.p99Latency, 0) /
      recentMetrics.length;
    ...
    if (latencyFactor > SIGNAL_THRESHOLDS.REGRESSION_LATENCY_FACTOR) {
      ...
      const signal: RegressionSignal = {
        ...
        recommendation:
          'Review PIPELINE_RUN events and runtime metrics. Check for resource contention.',
        regressionMetrics: {
    ...
    if (successDifference > SIGNAL_THRESHOLDS.REGRESSION_SUCCESS_DECLINE) {
      ...
      const signal: RegressionSignal = {
        ...
        recommendation:
          'Analyze recent errors and failures. Check governance signals (Phase 24) for policy changes.',
        regressionMetrics: {
          baseline_latency: context.baselineMetrics.latency,
          current_latency: avgLatency,
          latency_increase_factor: latencyFactor,
   ...
   */
  private detectOpportunitySignals(context: SignalDetectionContext): OpportunitySignal[] {
    ...
    const avgSuccessRate =
      ...
      recentMetrics.length;
    const consistency =
      recentMetrics.reduce((sum, m) => sum + (m.successRate > 0.95 ? 1 : 0), 0) /
      recentMetrics.length;

    if (
      avgSuccessRate > SIGNAL_THRESHOLDS.OPPORTUNITY_SUCCESS_RATE &&
      ...
      const signal: OpportunitySignal = {
        ...
        recommendation:
          'Consider accelerating dependent phases or increasing resource allocation.',
        opportunityMetrics: {
          success_rate: avgSuccessRate,
          consistency,
          potential_improvement:
            'Accelerate Phase 25–27 (APR/CRO/CKG) to capitalize on stability.',
        },
        metadata: { window: '7d', metric_count: recentMetrics.length },
      };
   ...
   */
  private averageDrift(metrics: DriftMetric[]) {
    const avg = {
      semantic_drift: 0,
      temporal_drift: 0,
      narrative_drift: 0,
   ...
   */
  private calculateFailureRate(metrics: HealthMetric[]): number {
   ...
   */
  private calculateLatencyVariance(metrics: HealthMetric[]): number {
   ...
   */
  private calculateConfidence(eventCount: number, threshold: number): number {
   ...
   */
  private extractAffectedPhases(events: TimelineEvent[]): string[] {
