...
export function createLearnerRouter(learner: AutonomyLearner): Router {
   ...
   */
  router.get('/learner/metrics', (_req: Request, res: Response) => {
    try {
   ...
   */
  router.get('/learner/thresholds', (_req: Request, res: Response) => {
    try {
   ...
   */
  router.get('/learner/accuracy/:signalType', (req: Request, res: Response) => {
    try {
      ...
      console.error(
        `GET /autonomy/learner/accuracy/${req.params.signalType} error:`,
        err
      );
      return res.status(500).json({
        error: err instanceof Error ? err.message : 'Unknown error',
   ...
   */
  router.post('/learner/feedback', async (req: Request, res: Response) => {
    try {
      ...
      const mockProposal: RoadmapProposal = {
        id: proposalId,
   ...
   */
  router.post('/learner/decay', async (req: Request, res: Response) => {
    try {
   ...
   */
  router.get('/learner/outcomes', (req: Request, res: Response) => {
    try {
   ...
   */
  router.get('/learner/summary', (_req: Request, res: Response) => {
    try {
      const metrics = learner.getMetrics();
      const thresholds = learner.getCurrentThresholds();
