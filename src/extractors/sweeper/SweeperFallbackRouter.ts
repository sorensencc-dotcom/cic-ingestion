...
import { CloakBrowserAdapter } from '../browser/CloakBrowserAdapter';
import { IBrowserEngine, BrowserSession } from '../browser/IBrowserEngine';
...
export class SweeperFallbackRouter {
  ...
  private routingMetrics: Map<string, { attempts: number; successes: number }> =
    new Map();
   ...
   */
  async ingest(url: string, opts?: { vertical?: Vertical }): Promise<IngestionResult> {
   ...
   */
  private async ingestionWithBrowserFirst(
    url: string,
    vertical: Vertical,
    startTime: number
    ...
    try {
      ...
      const err = browserError as Error & { code?: string };
      ...
      try {
        ...
        const htmlErr = htmlError as Error;
        result.success = false;
        result.method = 'dlq';
   ...
   */
  private async ingestionWithHTMLFirst(
    url: string,
    vertical: Vertical,
    startTime: number
  ): Promise<IngestionResult> {
    ...
    try {
      ...
      try {
        ...
        // Both failed: route to DLQ
        const browserErr = browserError as Error & { code?: string };
        result.success = false;
        result.method = 'dlq';
        ...
        this.routeToDLQ(
          ...
          vertical,
          'html',
          browserErr.code || 'ALL_METHODS_FAILED',
          browserErr.message
        );
   ...
   */
  private detectVertical(url: string): Vertical {
    const lower = url.toLowerCase();

    // Dental
    if (
      /dent|orthodont|smile|teeth|implant|braces/i.test(lower)
    ) {
    ...
    // Trades
    if (
      /plumber|electrician|hvac|contractor|carpenter|painter|trades/i.test(
        lower
      )
    ) {
   ...
   */
  private isHighJSVertical(vertical: Vertical): boolean {
    return [
      Vertical.DENTAL,
      Vertical.MEDSPA,
      Vertical.MEDICAL,
   ...
   */
  getMetrics(): Record<string, { attempts: number; successes: number }> {
    const metrics: Record<string, { attempts: number; successes: number }> = {};
