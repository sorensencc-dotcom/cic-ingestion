...
function testWaveBRouting(): void {
  ...
  recordIngestion(
    { id: "wave-b-test", source: "test.com" },
    { profile: "api", lane: "fast", extractors: ["TextExtractor"] },
...
function testWaveCDaemonRouting(): void {
...
function testWaveDQuarantine(): void {
  ...
  recordIngestion(
    { id: "quarantine-1", source: "test.com" },
    { profile: "api", lane: "quarantine", extractors: [] },
    { passed: false, errors: ["Network timeout"], cost: 5 },
    { extractorCost: 5, verificationCost: 0, totalCost: 5 }
  );
...
function testWaveEDurability(): void {
  ...
  // Add old record
  const oldRecord: ManifestRecord = {
    id: "old-1",
    source: "test.com",
    mediaType: "text/plain",
    profile: "api",
...
function testWaveFEndToEnd(): void {
  ...
  for (const line of remaining) {
    try {
      ...
      assert(
        rec.id && rec.source && rec.timestamps,
        `Remaining record ${rec.id} is valid`
      );
    } catch (e) {
...
function testRecoveryScenarios(): void {
  ...
  recordIngestion(
    { id: "boundary-keep", source: "test.com" },
    { profile: "api", lane: "fast", extractors: [] },
    { passed: true, errors: [], cost: 5 },
    { extractorCost: 5, verificationCost: 0, totalCost: 5 }
  );
