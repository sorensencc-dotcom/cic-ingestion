...
import { FileLockedError, } from "./types";
...
export function recordIngestion(entry, decision, verification, cost, routingVersion) {
    // Acquire lock with timeout
    const lockAcquired = acquireLock();
    if (!lockAcquired) {
        throw new FileLockedError(`Failed to acquire manifest lock after ${LOCK_TIMEOUT_MS}ms`);
    }
    try {
...
export function backfillFromProcessedLines(processedLines) {
    for (const line of processedLines) {
        ...
        recordIngestion(line, { profile: "filesystem", lane: "fast", extractors: line.extractors || [] }, verification, backfillCost, "legacy");
    }
}
