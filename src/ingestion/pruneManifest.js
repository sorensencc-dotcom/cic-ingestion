...
function getTimestampDate(record) {
    const timestamp = record.timestamps?.ingested ||
...
export function pruneManifest(retentionDays = RETENTION_DAYS) {
    // Acquire lock
...
export function getPruneStats(retentionDays = RETENTION_DAYS) {
    if (!fs.existsSync(MANIFEST_PATH)) {
        return {
            totalRecords: 0,
            retainedRecords: 0,
