...
export function listQuarantined() {
    ...
    return records.filter((r) => r.lane === "quarantine" || r.operatorFlags?.quarantine === true);
}
// Get single quarantined item by ID
export function getQuarantined(id) {
    const records = loadManifest();
    return records.find((r) => r.id === id && (r.lane === "quarantine" || r.operatorFlags?.quarantine === true));
}
// Approve a quarantined item (mark for reingest, clear quarantine)
export function approveQuarantine(id, targetLane = "fast") {
    ...
    record.lane = targetLane;
    // Append updated record
    appendRecord(record);
}
// Reject a quarantined item (mark as skip)
export function rejectQuarantine(id, reason) {
    const record = getQuarantined(id);
    if (!record) {
        throw new Error(`Quarantined record not found: ${id}`);
    }
