...
export class ExecutionOrchestrator {
    ...
    try {
      ...
      if (!adapterDef) {
        return this.createFailedResult(
          ...
          'failed',
          'ADAPTER_NOT_FOUND',
          `Adapter ${adapterId} not found in registry`,
          startTime,
          traceId
        );
      }
      ...
      if (!authResult.allowed) {
        ...
        return this.createFailedResult(
          ...
          'denied',
          'POLICY_VIOLATION',
          authResult.reason || 'Authorization denied',
          startTime,
          traceId,
          {
            authorized: false,
      ...
      if (!callsOk.ok) {
        return this.createFailedResult(
          ...
          'denied',
          'LIMIT_EXCEEDED',
          `Call limit exceeded: ${callsOk.current}/${callsOk.limit}`,
          startTime,
          traceId
        );
      }
      ...
      if (needsApproval && context.approvalGate) {
        try {
          const approvalResult = await context.approvalGate.request(
            context.agent,
            adapterId,
            input
          );
          ...
          if (!approvalResult.approved) {
            ...
            return this.createFailedResult(
              ...
              'denied',
              'APPROVAL_DENIED',
              approvalResult.reason || 'Operation not approved',
              startTime,
              traceId,
              {
                authorized: true,
      ...
      try {
        ...
        return this.createFailedResult(
          ...
          'failed',
          'SANDBOX_ERROR',
          `Failed to create sandbox: ${err.message}`,
          startTime,
          traceId
        );
      }
  ...
  async bulkExecute(
    ...
    const results = await Promise.all(
      operations.map(op => this.execute(op.adapterId, op.input, op.context))
    );
