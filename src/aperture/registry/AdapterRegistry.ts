 ...
 */
function validateAgainstSchema(input: any, schema: any): { valid: boolean; errors?: string[] } {
  const errors: string[] = [];
...
export class AdapterRegistry {
  private adapters: Map<string, AdapterDefinition> = new Map();
   ...
   */
  validateOutput(id: string, output: any): { valid: boolean; errors?: string[] } {
    const adapter = this.lookup(id);
    if (!adapter) {
      return {
        valid: false,
 ...
 */
export function createV1Registry(): AdapterRegistry {
